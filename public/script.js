window.fathom = (function () {
	var fathomScript =
		...
		excludedDomains = [],
		allowedDomains = [];
	'true' == (fathomScript.getAttribute('data-honor-dnt') || fathomScript.getAttribute('honor-dnt')) &&
		...
		'false' == (fathomScript.getAttribute('data-canonical') || fathomScript.getAttribute('canonical')) &&
			(canonical = !1),
		(fathomScript.getAttribute('data-excluded-domains') || fathomScript.getAttribute('excluded-domains')) &&
			(excludedDomains = (
				fathomScript.getAttribute('data-excluded-domains') || fathomScript.getAttribute('excluded-domains')
			).split(',')),
		fathomScript.getAttribute('data-included-domains') || fathomScript.getAttribute('included-domains')
			? (allowedDomains = (
					fathomScript.getAttribute('data-included-domains') || fathomScript.getAttribute('included-domains')
			  ).split(','))
			: (fathomScript.getAttribute('data-allowed-domains') || fathomScript.getAttribute('allowed-domains')) &&
			  (allowedDomains = (
					fathomScript.getAttribute('data-allowed-domains') || fathomScript.getAttribute('allowed-domains')
			  ).split(','));
	...
	function spaHistory() {
		...
		void 0 !== history &&
			...
			(history.pushState = function () {
				...
				return (
					window.dispatchEvent(new Event('pushstate')), window.dispatchEvent(new Event('locationchangefathom')), ret
				);
			}),
	...
	function encodeParameters(params) {
		return (params.cid = Math.floor(1e8 * Math.random()) + 1), '?v=' + encodeURIComponent(btoa(JSON.stringify(params)));
	}
	function qs() {
		for (
			var pair,
				data = {},
				pairs = window.location.search.substring(window.location.search.indexOf('?') + 1).split('&'),
				i = 0;
			i < pairs.length;
			i++
		)
			pairs[i] &&
				...
				-1 <
					[
						...
						'pagename',
						'tab'
					].indexOf(decodeURIComponent(pair[0])) && (data[decodeURIComponent(pair[0])] = decodeURIComponent(pair[1])));
		return data;
	}
		...
		return (
			void 0 === params.url
				? canonical &&
				  ...
				  (((a = document.createElement('a')).href = document.querySelector('link[rel="canonical"][href]').href),
				  (location = a))
	...
	return (
			...
			}),
		{
			...
			enableTrackingForMe: function () {
				window.localStorage &&
					(window.localStorage.removeItem('blockFathomTracking'), alert('Fathom has been enabled for this website.'));
			}
		}
	);
})();
